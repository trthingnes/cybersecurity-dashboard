...
export const useGetHaDumpEnvSuspense = <TData = Common.GetHaDumpEnvDefaultResponse, TError = GetHaDumpEnvError, TQueryKey extends Array<unknown> = unknown[]>(clientOptions: Options<unknown, true> = {}, queryKey?: TQueryKey, options?: Omit<UseQueryOptions<TData, TError>, "queryKey" | "queryFn">) => useSuspenseQuery<TData, TError>({ queryKey: Common.UseGetHaDumpEnvKeyFn(clientOptions, queryKey), queryFn: () => getHaDumpEnv({ ...clientOptions }).then(response => response.data as TData) as TData, ...options });
